import { m as restoreTerminalState, p as defaultRuntime } from "./subsystem-BKSEV_2a.js";
import { u as resolveGatewayPort } from "./paths-dQ_clcF4.js";
import { Ct as ensureWorkspaceAndSessions, Et as handleReset, Nt as randomToken, Ot as normalizeGatewayTokenInput, Pt as resolveControlUiLinks, Rt as waitForGatewayReachable, bt as applyWizardMetadata, l as readConfigFileSnapshot, p as writeConfigFile, yt as DEFAULT_WORKSPACE } from "./send-C3_deGKY.js";
...
import "./daemon-runtime-CjlHpTU-.js";
...
async function collectGatewayHealthFailureDiagnostics() {
	const diagnostics = {};
	try {
		const { resolveGatewayService } = await import("./service-CXKirQDx.js").then((n) => n.r);
	...
	try {
		const { readLastGatewayErrorLine } = await import("./diagnostics-BYQRViUe.js").then((n) => n.t);
...
async function runNonInteractiveOnboardingLocal(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-DGBNEvSD.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-DPGbPVdq.js");
