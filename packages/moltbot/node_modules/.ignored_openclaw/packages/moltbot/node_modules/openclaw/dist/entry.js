#!/usr/bin/env node
import { $ as isRootHelpInvocation, Z as hasHelpOrVersion, et as isRootVersionInvocation, t as createSubsystemLogger } from "./subsystem-BKSEV_2a.js";
...
if (!isMainModule({
	...
	function tryHandleRootVersionFastPath(argv) {
		...
		Promise.all([import("./version-D0tcvzW0.js").then((n) => n.r), import("./git-commit-B0vIx0co.js").then((n) => n.t)]).then(([{ VERSION }, { resolveCommitHash }]) => {
	...
	function tryHandleRootHelpFastPath(argv) {
		...
		import("./program-BQZa-m-Y.js").then(({ buildProgram }) => {
	...
	if (!ensureExperimentalWarningSuppressed()) {
		...
		if (!tryHandleRootVersionFastPath(process$1.argv) && !tryHandleRootHelpFastPath(process$1.argv)) import("./run-main-X_cBLt1N.js").then(({ runCli }) => runCli(process$1.argv)).catch((error) => {
