...
import { Ct as applyWizardMetadata, Et as ensureWorkspaceAndSessions, It as randomToken, Lt as resolveControlUiLinks, St as DEFAULT_WORKSPACE, Vt as waitForGatewayReachable, jt as normalizeGatewayTokenInput, kt as handleReset, m_ as readConfigFileSnapshot, v_ as writeConfigFile } from "./reply-cpXZjA1O.js";
...
async function collectGatewayHealthFailureDiagnostics() {
	const diagnostics = {};
	try {
		const { resolveGatewayService } = await import("./service-RXVSlLiU.js").then((n) => n.r);
	...
	try {
		const { readLastGatewayErrorLine } = await import("./diagnostics-1AwcyuKi.js").then((n) => n.t);
...
async function runNonInteractiveOnboardingLocal(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-Dj9v2i_J.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-CBeAo4lv.js");
	...
	if (!opts.skipHealth) {
		const { healthCommand } = await import("./health-Dt7huLJe.js").then((n) => n.i);
