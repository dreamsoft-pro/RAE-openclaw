...
export function createExecTool(
  ...
  return {
    ...
    execute: async (_toolCallId, args, signal, onUpdate) => {
      ...
      globalGuard.enforceHardFrames(params.command, {
        operationName: "bash_exec",
        impactLevel: "high",
        infoClass: "restricted"
      });

      const run = await globalGuard.auditedOperation(
        { operationName: "bash_exec", impactLevel: "high", infoClass: "restricted" },
        async () => {
