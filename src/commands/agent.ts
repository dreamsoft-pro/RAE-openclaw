...
async function fetchRaeContext(body: string, project: string = "dreamsoft_factory"): Promise<string> {
  try {
    const res = await fetch(`http://rae-api-dev:8000/v2/memories/query`, {
      method: "POST",
      headers: { 
        "Content-Type": "application/json", 
        "X-Tenant-Id": process.env.RAE_TENANT_ID || "53717286-fe94-4c8f-baf9-c4d2758eb672" 
      },
      body: JSON.stringify({ query: body, project, limit: 3 })
    });
    if (!res.ok) return "";
    const data = await res.json();
    if (!data.memories || data.memories.length === 0) return "";
    
    return "\n\n--- RAE ENTERPRISE KNOWLEDGE CONTEXT ---\n" + data.memories.map((m: any) => m.content).join("\n---\n") + "\n----------------------------------------\n\n";
  } catch (e) {
    return "";
  }
}
...
async function agentCommandInternal(
  ...
  // RAE-FIRST CONTEXT INJECTION
  const raeContext = await fetchRaeContext(opts.body || "");
  if (raeContext) {
     opts.extraSystemPrompt = (opts.extraSystemPrompt || "") + raeContext;
  }
